      ...
      <nav>
        <div className="lg:max-w-7xl lg:mx-auto px-4 lg:px-8">
          <div className="flex justify-between h-16">
            <img src={logo} className=" font-bold h-12 mt-2" />
            <div
            ...
            >
              ...
              <Link to="/services">
                <p className="px-7 py-3 rounded-full  hover:bg-sky-400 font-bold hover:text-black">
                  Services
                </p>
              </Link>
              <Link to="/features">
              ...
              <div className="flex items-center">
                {!isModalOpen ? (
                  <button className="lg:block " onClick={toggleModal}>
                    <BsSearch className=" mt-1 text-2xl font-bold ml-7" />
                  </button>
                ) : (
                  <button className="lg:block hidden" onClick={toggleModal}>
                    <BsX className=" mt-4 text-2xl font-bold ml-1" />
                  </button>
                )}
              </div>
            ...
            <button className="lg:hidden block" onClick={toggleMenu}>
              <svg className="w-6 h-6 fill-current text-black hover:text-black" viewBox="0 0 24 24">
                {isOpen ? (
                  <path
                    fill="#000000"
                    fillRule="evenodd"
      ...
      {isModalOpen && (
        <div className="fixed top-0 left-0 right-0 bottom-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 w-full max-w-screen-xl h-5/6">
            ...
            <button className="lg:block  " onClick={toggleModal}>
              <BsX className="  text-6xl font-bold -ml-6 -mt-6  " />
            </button>
